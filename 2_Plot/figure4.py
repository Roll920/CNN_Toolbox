...
def main():
    my_font = matplotlib.rcParams.update({'font.size': 20, 'font.family': 'STIXGeneral', 'mathtext.fontset': 'stix'})
    x, y = load_data()

    fig, ax = pl.subplots(1, 1)
    ax.plot(x.reshape(-1), y[0, :].reshape(-1), '^:', color='#0348a6', markersize=13, linewidth=4, label='Random')
    ax.plot(x.reshape(-1), y[1, :].reshape(-1), 's-.', color='#0ba040', markersize=13, linewidth=4, label='Weight sum')
    ax.plot(x.reshape(-1), y[2, :].reshape(-1), 'd--', color='#ed3bd6', markersize=13, linewidth=4, label='APoZ')
    ax.plot(x.reshape(-1), y[3, :].reshape(-1), 'ro--', markersize=13, linewidth=4, label='ThiNet w/o $\mathbf{\hat{w}}$')
    ax.plot(x.reshape(-1), y[4, :].reshape(-1), 'ro-', markersize=13, linewidth=4, label='ThiNet')
